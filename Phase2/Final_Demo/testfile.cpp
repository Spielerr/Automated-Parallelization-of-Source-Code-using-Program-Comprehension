...
vector<vector<string>> args{{"match_1_scores","match_1_len"}, {"match_2_scores","match_2_len"}, {"match_3_scores","match_3_len"}, {"match_1_scores","count_1","match_1_len"}, {"match_2_scores","count_2","match_2_len"}, {"match_3_scores","count_3","match_3_len"}, {"match_1_scores","match_1_len","match_2_scores","match_2_len","match_3_scores","match_3_len","merged_arr"}, {"merged_arr","n"}, {"merged_arr","n","0","99","0"}, {"merged_arr","0","range1_ind","arr1","100"}, {"merged_arr","n","100","199","range1_ind + 1"}, {"merged_arr","range1_ind + 1","range2_ind","arr2","100"}, {"merged_arr","n","200","300","range2_ind + 1"}, {"merged_arr","range2_ind + 1","range3_ind","arr3","101"}, {"arr1","n1","arr2","n2","arr3","n3","final_results"}, {"final_results","combined_length"}};
...
int main(int argc, char **argv)
{
    ...
    int match_1_len = 25000;
    int match_2_len = 35000;
    int match_3_len = 45000;
	...
	find_future(15, combined_length);
	auto fn16 = bind(::display, final_results, combined_length);
	{
		lock_guard<mutex> lockGuard(m_sp);
		iter1 = find_if(begin(special), end(special), Find_special2("final_results"));
		iter2 = end(special);
	}
	if(iter1 != iter2)
	{
		lock_guard<mutex> lockGuard(m_wq);
		wait_queue.push_back(pair<int, std::function<void()>>(16, fn16));
	}
	else
	{
		lock_guard<mutex> lockGuard(m_rq);
		ready_queue.push_back(pair<int, std::function<void()>>(16, fn16));
	}
