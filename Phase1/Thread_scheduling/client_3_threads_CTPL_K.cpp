	...
	while(flag)
	{
		...
		if(ready_queue.size() != 0)
		{
			for(auto x:ready_queue)
			{
				auto f = x.second;
				// cerr<<"ADDING\n\n";
				// void_futures.emplace_back(tp.push([](int){f();}));
				void_futures.emplace_back(tp.push(f));
...
int main(int argc, char **argv)
{
	...
	{
		wait_queue.push_back(pair<int, std::function<void(int)>>(2, fn2));
		// wait_queue.push_back(pair<int, std::function<void(int)>>(2, [&](int){ min1 = fn2(); }));
	}
	// push to wait queue
	else
	{
		ready_queue.push_back(pair<int, std::function<void(int)>>(2, fn2));
	...
	if(find_if(begin(special), end(special), Find_special2("arr1")) != end(special))
	{
		wait_queue.push_back(pair<int, std::function<void(int)>>(3, fn3));
		// ready_queue.push_back(pair<int, std::function<void(int)>>(3, [&](int){ max1 = fn3(); }));
	}
	else
	{
		ready_queue.push_back(pair<int, std::function<void(int)>>(3, fn3));
		// ready_queue.push_back(pair<int, std::function<void(int)>>(3, [&](int){ max1 = fn3(); } ));
	}
	...
	{
		//min to queue
		wait_queue.push_back(pair<int, std::function<void(int)>>(5, fn5));
		// ready_queue.push_back(pair<int, std::function<void(int)>>(5, [&](int){ min2 = fn5(); }));
	}
	else
	{
		ready_queue.push_back(pair<int, std::function<void(int)>>(5, fn5));
		// ready_queue.push_back(pair<int, std::function<void(int)>>(5, [&](int){ min2 = fn5(); } ));
	}
	...
	{
		//min to queue
		wait_queue.push_back(pair<int, std::function<void(int)>>(6, fn6));
		// ready_queue.push_back(pair<int, std::function<void(int)>>(6, [&](int){ max2 = fn6(); }));
	}
	else
	{
		ready_queue.push_back(pair<int, std::function<void(int)>>(6, fn6));
		// ready_queue.push_back(pair<int, std::function<void(int)>>(6, [&](int){ max2 = fn6(); } ));
	}
